...
pub trait Scene<SceneChangeParamsT, EngineDataT> {
    ...
    fn update(&mut self, ctx: &mut Context, engine_data: &mut EngineDataT) -> Option<SceneChangeEvent<SceneChangeParamsT>>;
    fn key_down_event(
        &mut self,
        ctx: &mut Context,
        keycode: KeyCode,
...
impl<'a, SceneChangeParamsT, EngineDataT> SceneStack<'a, SceneChangeParamsT, EngineDataT> {
    ...
    pub fn update(&mut self, ctx: &mut Context, engine_data: &mut EngineDataT)  -> Option<SceneChangeEvent<SceneChangeParamsT>> {
        let maybe_last_scene = self.scenes.pop();
        match maybe_last_scene {
            Some(mut scene) => {
